...
export default Gameboard = ({ route }) => {
  ...
  useEffect(() => {
    if (nbrOfThrowsLeft == 0) {
      ...
      totalPoints >= BONUS_POINTS_LIMIT
        ? setStatus("Congrats, you got the bonus ")
        : setStatus("No bonus for you!");
    }
  }, [nbrOfThrowsLeft]);
  ...
  function getDiceColor(i) {
    if (board.every((val, i, arr) => val === arr[0])) {
      ...
      return selectedDices[i] ? "black" : "#CD5C5C";
    }
  }

  function getDicePointsColor(i) {
    return selectedDicePoints[i] ? "black" : "#CD5C5C";
  }

  function selectDice(i) {
    if (nbrOfThrowsLeft == 3) {
      setStatus("Throw first before selecting!");
      return;
    }
  ...
  function selectDicePoints(i) {
    ...
    if (selectedDicePoints[i]) {
      setStatus("Points already selected, select a different number!");
      return;
    }
